...
function normalizeAndConvertToUnits(amount: Amount, currency: CurrencyISOCode): number {
    const normalized = roundToCurrencyPrecision(amount, currency);
    return amountToSmallestUnit(normalized, currency);
}

export function normalizeAmount(amount: Amount, currency: CurrencyISOCode): Amount {
...
export function addAmounts(a: Amount, b: Amount, currency: CurrencyISOCode): Amount {
...
export function subtractAmounts(a: Amount, b: Amount, currency: CurrencyISOCode): Amount {
...
export function negateAmount(amount: Amount, currency: CurrencyISOCode): Amount {
...
export function absAmount(amount: Amount, currency: CurrencyISOCode): Amount {
...
export function compareAmounts(a: Amount, b: Amount, currency: CurrencyISOCode): number {
    return normalizeAndConvertToUnits(a, currency) - normalizeAndConvertToUnits(b, currency);
}

export function minAmount(a: Amount, b: Amount, currency: CurrencyISOCode): Amount {
    return compareAmounts(a, b, currency) <= 0 ? normalizeAmount(a, currency) : normalizeAmount(b, currency);
}

export function sumAmounts(amounts: Array<Amount>, currency: CurrencyISOCode): Amount {
...
export function isZeroAmount(amount: Amount, currency: CurrencyISOCode): boolean {
