...
export class AppDriver {
    constructor(
        ...
        private readonly firestoreDb: admin.firestore.Firestore,
    ) {}
    ...
    async waitForMultiUserTransactionAndBalanceNotifications(groupId: string, userIds: string[], timeout = 5000) {
        await Promise.all(
            userIds.map((userId) =>
                Promise.all([this.waitForUserNotificationUpdate(userId, groupId, 'transaction', timeout), this.waitForUserNotificationUpdate(userId, groupId, 'balance', timeout)]),
            ),
        );
    }
    ...
    async waitForMultiUserNotificationUpdate(userIds: string[], groupId: string, changeType: 'transaction' | 'balance' | 'group', timeout = 3000): Promise<void> {
        await Promise.all(userIds.map((userId) => this.waitForUserNotificationUpdate(userId, groupId, changeType, timeout)));
    }
    ...
    async waitForAllNotifications(userId: string, groupId: string, changeTypes: ('transaction' | 'balance' | 'group')[], timeout = 3000): Promise<void> {
        await Promise.all(changeTypes.map((changeType) => this.waitForUserNotificationUpdate(userId, groupId, changeType, timeout)));
    }
    ...
    async countGroupChanges(groupId: string): Promise<number> {
        ...
        for (const doc of snapshot.docs) {
            ...
            if (data.recentChanges) {
                totalChanges += data.recentChanges.filter((change) => change.groupId === groupId && change.type === 'group').length;
            }
        }
    ...
    async countExpenseChanges(groupId: string): Promise<number> {
        ...
        for (const doc of snapshot.docs) {
            const data = doc.data() as UserNotificationDocument;
            if (data.recentChanges) {
                totalChanges += data.recentChanges.filter((change) => change.groupId === groupId && change.type === 'transaction').length;
            }
        }
    ...
    async countBalanceChanges(groupId: string): Promise<number> {
        ...
        for (const doc of snapshot.docs) {
            const data = doc.data() as UserNotificationDocument;
            if (data.recentChanges) {
                totalChanges += data.recentChanges.filter((change) => change.groupId === groupId && change.type === 'balance').length;
            }
        }
