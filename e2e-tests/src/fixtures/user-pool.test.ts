...
describe('UserPool Singleton Pattern', () => {
  ...
  it('should throw error when trying to create second instance', () => {
    ...
    expect(() => new UserPool()).toThrow(
      'UserPool has already been instantiated! Use getUserPool() to get the existing instance. Multiple UserPool instances would cause user conflicts.'
    );
  });
