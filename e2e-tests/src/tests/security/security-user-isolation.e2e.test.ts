...
test.describe('User Storage Isolation', () => {
  ...
  multiUserTest('should isolate user preferences between different users', async ({ 
    authenticatedPage, 
    secondUser 
  }) => {
  ...
  multiUserTest('should clear user storage on logout', async ({ 
    authenticatedPage 
  }) => {
    ...
    await page.waitForURL((url: string) => !url.toString().includes('/dashboard'), { timeout: 5000 });
