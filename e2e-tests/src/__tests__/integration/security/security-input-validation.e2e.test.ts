...
test.describe('Security Input Validation Tests', () => {
        ...
        test('prevents XSS in user profile fields', async ({ authenticatedPage }) => {
            ...
            if (await profileButton.isVisible()) {
                ...
                if (await profileLink.isVisible()) {
                    ...
                    const xssPayloads = ['<script>alert("profile-xss")</script>', '<img src=x onerror="alert(\'profile\')">', 'javascript:alert("profile")'];
    ...
    test.describe('SQL Injection Prevention', () => {
        ...
        test('prevents SQL injection in expense filters', async ({ authenticatedPage }) => {
            ...
            const sqlPayloads = ["' OR 1=1--", "'; DELETE FROM groups WHERE id='", "' UNION SELECT password FROM users--", "1' AND (SELECT COUNT(*) FROM sensitive_table) > 0--"];
                    ...
                    try {
                        ...
                        if (page.url().includes('/add-expense')) {
                            ...
                            if (await receiptElement.isVisible()) {
                                const receiptUrl = (await receiptElement.getAttribute('src')) || (await receiptElement.getAttribute('href'));
        ...
        if (await loginForm.isVisible()) {
            ...
            try {
                ...
                cookies.forEach((cookie) => {
                    if (cookie.name.toLowerCase().includes('session') || cookie.name.toLowerCase().includes('auth') || cookie.name.toLowerCase().includes('token')) {
