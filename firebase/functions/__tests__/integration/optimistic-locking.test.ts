...
import { SplitTypes } from '../../src/shared/shared-types';
  ...
  describe('Group Optimistic Locking', () => {
    ...
    test('should prevent concurrent group updates', async () => {
      ...
      const updatePromises = [
        driver.updateGroup(group.id, { name: 'Updated by User 1' }, users[0].token),
        driver.updateGroup(group.id, { name: 'Updated by User 2' }, users[1].token),
      ];
  ...
  describe('Expense Optimistic Locking', () => {
    test('should prevent concurrent expense updates', async () => {
      ...
      console.log('Update results:', results.map(r => 
        r.status === 'rejected' ? 
          { status: 'rejected', error: r.reason?.response?.data?.error || r.reason?.message } : 
          { status: 'fulfilled' }
      ));
    ...
    test('should prevent concurrent expense deletion', async () => {
      ...
      // Log the actual errors for debugging
      console.log('Delete/Update results:', results.map(r => 
        r.status === 'rejected' ? 
          { status: 'rejected', error: r.reason?.response?.data?.error || r.reason?.message } : 
          { status: 'fulfilled' }
      ));
