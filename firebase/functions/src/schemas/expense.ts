...
}).merge(AuditFieldsSchema).merge(SoftDeletionFieldsSchema);

/**
 * Create Document and Data schemas using common pattern
 */
const { DocumentSchema: ExpenseDocumentSchema, DataSchema: ExpenseDataSchema } = 
    createDocumentSchemas(BaseExpenseSchema);
 ...
 * ```
 */
export { ExpenseDocumentSchema, ExpenseDataSchema };
