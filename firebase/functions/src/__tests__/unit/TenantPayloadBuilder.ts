 ...
 * Can build both TenantDocumentUpsertData (for FirestoreWriter) and AdminUpsertTenantRequest (for admin API).
 * Provides sensible defaults for all the extensive branding and token configuration.
 */
export class TenantPayloadBuilder {
    private payload: TenantDocumentUpsertData;
    private tenantId: TenantId;

    constructor(tenantId?: string) {
        this.tenantId = toTenantId(tenantId || 'test-tenant');
        this.payload = {
            branding: {
            ...
            domains: [toTenantDomainName('foo'), toTenantDomainName('bar')],
            defaultTenant: toTenantDefaultFlag(false),
        };
    }

    withTenantId(tenantId: TenantId | string): this {
        this.tenantId = typeof tenantId === 'string' ? toTenantId(tenantId) : tenantId;
        return this;
    }

    withAppName(appName: string): this {
        this.payload.brandingTokens!.tokens.legal.appName = appName;
        return this;
    }
    ...
    /**
     * Builds TenantDocumentUpsertData for use with FirestoreWriter.upsertTenant().
     * Does NOT include tenantId (it's passed separately to upsertTenant).
     */
    build(): TenantDocumentUpsertData {
     ...
     */
    static forTenant(tenantId: string): TenantPayloadBuilder {
        return new TenantPayloadBuilder(tenantId);
    }
}
