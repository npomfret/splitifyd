...
describe('app tests', () => {
    ...
    describe('happy path tests', async () => {
        ...
        it('should preserve expense metadata and remove it cleanly on deletion', async () => {
            ...
            const metadataExpense = await appDriver.createExpense(new CreateExpenseRequestBuilder()
                .withGroupId(groupId)
    ...
    describe('edge cases', () => {
        ...
        it('should reject expense updates when participants include non-members', async () => {
            ...
            const baseExpense = await appDriver.createExpense(new CreateExpenseRequestBuilder()
                .withGroupId(groupId)
            ...
            await expect(appDriver.updateExpense(baseExpense.id, ExpenseUpdateBuilder
                .minimal()
                .withAmount(90, 'USD')
                .withParticipants(updatedParticipants)
                .withSplits(updatedSplits)
        ...
        describe('boundary and limit testing', () => {
            ...
            it('should enforce maximum length on expense description', async () => {
                ...
                await expect(
                    appDriver.createExpense(new CreateExpenseRequestBuilder()
                        .withGroupId(groupId)
                        .withDescription(longDescription)
                        .withAmount(100, 'USD')
                        .withPaidBy(user1)
    ...
    describe('Admin Tenant Management', () => {
        ...
        describe.skip('POST /api/admin/tenants/publish - publishTenantTheme', () => {
            const createTenantWithTokens = async (tenantId: string) => {
                ...
                await appDriver.upsertTenant(adminUser, payload);
            };

            it('should publish theme artifacts for tenant with tokens', async () => {
                const tenantId = 'tenant_with_tokens';
                await createTenantWithTokens(tenantId);

                const result = await appDriver.publishTenantTheme(adminUser, { tenantId });
            ...
            it('should update artifact metadata in tenant document', async () => {
                const tenantId = 'tenant_artifact_metadata';
                await createTenantWithTokens(tenantId);
            ...
            it('should increment version on subsequent publishes', async () => {
                const tenantId = 'tenant_version_increment';
                await createTenantWithTokens(tenantId);

                const result1 = await appDriver.publishTenantTheme(adminUser, { tenantId });
                expect(result1.artifact.version).toBe(1);

                const result2 = await appDriver.publishTenantTheme(adminUser, { tenantId });
                expect(result2.artifact.version).toBe(2);

                const result3 = await appDriver.publishTenantTheme(adminUser, { tenantId });
                expect(result3.artifact.version).toBe(3);
            });

            it('should reject request for non-existent tenant', async () => {
                const result = await appDriver.publishTenantTheme(adminUser, { tenantId: 'nonexistent_tenant' });
            ...
            it('should reject request for tenant without branding tokens', async () => {
                // Create tenant without brandingTokens
                const tenantId = 'tenant_no_tokens';
                await appDriver.upsertTenant(adminUser, {
                    tenantId,
                    branding: {
                        appName: 'No Tokens Tenant',
                        logoUrl: 'https://static.splitifyd.dev/branding/test/logo.svg',
                        faviconUrl: 'https://static.splitifyd.dev/branding/test/favicon.png',
                        primaryColor: '#2563eb',
                        secondaryColor: '#7c3aed',
                    ...
                    domains: {
                        primary: 'notoken.example.com',
                        aliases: [],
                        normalized: ['notoken.example.com'],
                    },
                    features: {
                        enableAdvancedReporting: false,
                        enableMultiCurrency: true,
                ...
                expect(result).toMatchObject({
                    error: {
                        statusCode: 400,
                        code: 'TENANT_TOKENS_MISSING',
                    },
                });
            });

            it('should reject request without tenant ID', async () => {
                const result = await appDriver.publishTenantTheme(adminUser, {} as any);
            ...
            it('should reject request from non-admin user', async () => {
                const tenantId = 'tenant_auth_check';
                await createTenantWithTokens(tenantId);

                const regularUser = 'regular-user';
                const result = await appDriver.publishTenantTheme(regularUser, { tenantId });
