...
describe('GroupCommentStrategy', () => {
    ...
    describe('verifyAccess', () => {
        it('should allow access when group exists and user is member', async () => {
            ...
            await expect(
                strategy.verifyAccess('test-group', 'user-id')
        ...
        it('should throw NOT_FOUND when group does not exist', async () => {
            ...
            await expect(
                strategy.verifyAccess('nonexistent-group', 'user-id')
            ).rejects.toThrow(ApiError);

            const error = await strategy.verifyAccess('nonexistent-group', 'user-id')
        ...
        it('should throw FORBIDDEN when user is not a group member', async () => {
            ...
            await expect(
                strategy.verifyAccess('test-group', 'unauthorized-user')
            ).rejects.toThrow(ApiError);

            const error = await strategy.verifyAccess('test-group', 'unauthorized-user')
                .catch((e: ApiError) => e) as ApiError;
