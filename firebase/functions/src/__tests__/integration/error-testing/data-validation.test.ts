...
describe('Enhanced Data Validation Tests', () => {
    ...
    describe('Date Validation', () => {
        test('should reject expenses with future dates (security improvement)', async () => {
            ...
            await expect(driver.createExpense(expenseData, users[0].token)).rejects.toThrow(/Date cannot be in the future/);
        });
        ...
        test('should reject expenses with invalid date formats', async () => {
            ...
            await expect(driver.createExpense(expenseData, users[0].token)).rejects.toThrow(/invalid.*date|date.*format|INVALID_DATE/i);
        });
    ...
    describe('Category Validation', () => {
        ...
        test('should reject expenses with null or undefined categories', async () => {
            ...
            await expect(driver.createExpense(expenseData, users[0].token)).rejects.toThrow(/category.*required|MISSING_CATEGORY|INVALID_CATEGORY/i);
        });
        ...
        test('should reject expenses with empty string categories', async () => {
            ...
            await expect(driver.createExpense(expenseData, users[0].token)).rejects.toThrow(/category.*required|category.*empty|INVALID_CATEGORY/i);
        });
    ...
    describe('User Input Limits', () => {
        test('should reject expense descriptions that exceed maximum length', async () => {
            ...
            await expect(driver.createExpense(expenseData, users[0].token)).rejects.toThrow(/description.*required|INVALID_DESCRIPTION|length.*exceeded|TOO_LONG/i);
        });
        ...
        test('should reject empty or whitespace-only input fields', async () => {
            ...
            await expect(driver.createExpense(expenseData, users[0].token)).rejects.toThrow(/description.*required|description.*empty|INVALID_INPUT/i);
        });
