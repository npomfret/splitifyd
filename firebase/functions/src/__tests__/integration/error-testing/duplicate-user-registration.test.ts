...
describe('Duplicate User Registration Tests', () => {
  ...
  describe('Sequential Registration', () => {
    test('should prevent duplicate email registration', async () => {
      ...
      await expect(
        driver.register(userData)
      ).rejects.toThrow(/409|email.*exists|already.*registered/i);
    });
  ...
  describe('Concurrent Registration', () => {
    test('should handle concurrent registration attempts with same email', async () => {
      ...
      const promises = Array(5).fill(null).map(() => 
        driver.register(userData).catch(err => err)
      );
    ...
    test('should handle rapid sequential registrations with same email', async () => {
      ...
      for (let i = 0; i < attempts; i++) {
        await expect(
          driver.register(userData)
        ).rejects.toThrow(/409|email.*exists/i);
      }
    });
  });
  ...
  describe('Case Sensitivity', () => {
    test('should treat email addresses case-insensitively', async () => {
      ...
      const upperCaseData = new UserBuilder()
  ...
  describe('Edge Cases', () => {
    test('should handle registration with trimmed email addresses', async () => {
      ...
      const spacedData = new UserBuilder()
        .withEmail(`  ${baseEmail}  `)
        .withPassword(userData.password)
        .withDisplayName(userData.displayName)
        .build();
      ...
      await expect(
        driver.register(spacedData)
      ).rejects.toThrow(/400|invalid.*email|validation/i);
    });

    test('should allow different users with different emails', async () => {
      const timestamp = Date.now();
      const user1 = new UserBuilder()
        .withEmail(`user1-${timestamp}@example.com`)
        .build();
      const user2 = new UserBuilder()
        .withEmail(`user2-${timestamp}@example.com`)
        .build();
  ...
  describe('Error Recovery', () => {
    test('should allow registration after previous failure', async () => {
      ...
      await expect(
        driver.register(invalidData)
      ).rejects.toThrow(/400|password/i);
