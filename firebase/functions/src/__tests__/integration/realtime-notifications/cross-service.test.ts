...
describe('Cross-Service Integration Tests', () => {
    ...
    describe('Service Integration', () => {
        test('should integrate with balance calculation changes', async () => {
            ...
            const user1BalanceEvents = listener1.getEventsForGroup(testGroup.id).filter((e) => e.type === 'balance');
            const user2BalanceEvents = listener2.getEventsForGroup(testGroup.id).filter((e) => e.type === 'balance');
    ...
    describe('Feature Integration', () => {
        test('should integrate with group sharing and invitations', async () => {
            ...
            const user1GroupEvents = listener1.getEventsForGroup(testGroup.id).filter((e) => e.type === 'group');
            const user2GroupEvents = listener2.getEventsForGroup(testGroup.id).filter((e) => e.type === 'group');
