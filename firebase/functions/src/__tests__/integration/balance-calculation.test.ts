...
describe('Balance Calculation Integration Test', () => {
    ...
    it('should correctly calculate balances for multi-user expense sharing', async () => {
        console.log('Starting balance calculation integration test...');
        ...
        console.log('Share link created:', shareLinkId);

        // Step 3: User 2 joins via share link
        console.log('\nStep 3: User 2 joining via share link...');
        ...
        console.log('\nStep 4: User 1 adding first shared expense ($100)...');
        ...
        console.log('First expense added:', expense1Response.id);

        // Wait for balance calculation using proper polling
        console.log('\nWaiting for balance calculation after first expense...');
        const balancesAfterFirst = await apiDriver.waitForBalanceUpdate(groupId, user1.token, 5000);
        console.log('Balances after first expense:', JSON.stringify(balancesAfterFirst, null, 2));
        ...
        console.log('\nStep 5: User 2 adding second shared expense ($60)...');
        ...
        console.log('Second expense added:', expense2Response.id);

        // Wait for balance recalculation using polling
        console.log('\nWaiting for balance calculation after second expense...');
        ...
        console.log('Balances after second expense:', JSON.stringify(balancesAfterSecond, null, 2));
        ...
        console.log('\nStep 6: User 3 joining via share link...');
        await apiDriver.joinGroupViaShareLink(shareLinkId, user3.token);
        console.log('User 3 joined the group');

        // Step 7: User 3 adds an expense ($90, split equally among all three)
        console.log('\nStep 7: User 3 adding expense ($90, split among all three)...');
        ...
        console.log('Third expense added:', expense3Response.id);

        // Wait for final balance calculation using polling
        console.log('\nWaiting for final balance calculation...');
        ...
        console.log('\nVerifying individual debts matrix...');
        if (finalBalances.debtMatrix) {
            console.log('Debt matrix:', JSON.stringify(finalBalances.debtMatrix, null, 2));
        }

        // Get full group details to verify everything is consistent
        console.log('\nGetting full group details for final verification...');
        const fullDetails = await apiDriver.getGroupFullDetails(groupId, user1.token);
        console.log('Group balance from full details:', JSON.stringify(fullDetails.balances, null, 2));
        ...
        if (fullDetails.balances.balancesByCurrency?.USD) {
            const usdBalances = fullDetails.balances.balancesByCurrency.USD;
            expect(usdBalances[user1.uid].netBalance).toBe(-10);
            expect(usdBalances[user2.uid].netBalance).toBe(-50);
            expect(usdBalances[user3.uid].netBalance).toBe(60);
        } else {
