...
describe('Notifications Management - Consolidated Tests', () => {
    ...
    describe('Core Notification Document Operations', () => {
        ...
        test('should create and update notification documents for basic operations', async () => {
            ...
            const group = await apiDriver.createGroup(new CreateGroupRequestBuilder()
                .build(), user1.token);
            ...
            await apiDriver.createExpense(new CreateExpenseRequestBuilder()
        ...
        test('should handle notification version increments correctly', async () => {
            ...
            await apiDriver.updateGroup(group.id, new GroupUpdateBuilder()
        ...
        test('should handle rapid multiple updates without losing notifications', async () => {
            ...
            await Promise.all([
                apiDriver.updateGroup(group.id, new GroupUpdateBuilder()
                    .withName('Update 1')
                    .build(), user1.token),
                apiDriver.updateGroup(group.id, new GroupUpdateBuilder()
                    .withName('Update 2')
    ...
    describe('Multi-User Notification Distribution', () => {
        ...
        test('should handle member addition and removal notification patterns', async () => {
            ...
            const dynamicGroup = await apiDriver.createGroup(new CreateGroupRequestBuilder()
                .build(), user1.token);
    ...
    describe('Permission-Based Notification Access Control', () => {
        test('should only notify users for groups they have access to', async () => {
            ...
            const privateGroup = await apiDriver.createGroup(new CreateGroupRequestBuilder()
            ...
            const publicGroup = await apiDriver.createGroup(new CreateGroupRequestBuilder()
        ...
        test('should handle permission changes and notification access', async () => {
            ...
            const permissionGroup = await apiDriver.createGroup(new CreateGroupRequestBuilder()
                .build(), user1.token);
    ...
    describe('Different Event Types and Triggers', () => {
        test('should generate notifications for various operation types', async () => {
            ...
            // ACTION: Create group
            const eventGroup = await apiDriver.createGroup(new CreateGroupRequestBuilder()
                .build(), user1.token);
            ...
            await apiDriver.updateExpense(expense.id, new ExpenseUpdateBuilder()
                .withAmount(45.0)
                .withCurrency('USD')
                .build(), user1.token);
            ...
            await apiDriver.createSettlement(new CreateSettlementRequestBuilder()
                .withGroupId(eventGroup.id)
                .withPayerId(user2.uid)
                .withPayeeId(user1.uid)
                .build(),
        ...
        test('should handle balance change notifications', async () => {
            ...
            const balanceGroup = await apiDriver.createGroup(new CreateGroupRequestBuilder()
    ...
    describe('Listener Management and Connection Handling', () => {
        test('should handle listener subscription churn without missing events', async () => {
            ...
            // ACTION: Create group
            const churnGroup = await apiDriver.createGroup(new CreateGroupRequestBuilder()
                .build(), user1.token);
        ...
        test('should handle network interruption and recovery', async () => {
            ...
            // ACTION: Create group
            const recoveryGroup = await apiDriver.createGroup(new CreateGroupRequestBuilder()
                .build(), user1.token);
        ...
        test('should handle document version conflicts gracefully', async () => {
            ...
            // ACTION: Create group with user1 as creator
            const conflictGroup = await apiDriver.createGroup(new CreateGroupRequestBuilder()
        ...
        test('should handle document locking scenarios under load', async () => {
            ...
            // ACTION: Create group
            const lockingGroup = await apiDriver.createGroup(new CreateGroupRequestBuilder()
                .build(), user1.token);
    ...
    describe('Service Integration and Cross-Feature Testing', () => {
        ...
        test('should integrate with group policy and settings changes', async () => {
            ...
            // ACTION: Create group with user1 as creator
            const policyGroup = await apiDriver.createGroup(new CreateGroupRequestBuilder()
        ...
        test('should handle group sharing and invitation workflows', async () => {
            ...
            // ACTION: Create group
            const sharingGroup = await apiDriver.createGroup(new CreateGroupRequestBuilder()
                .build(), user1.token);
    ...
    describe('Notification System Performance and Edge Cases', () => {
        test('should handle notification system under high load', async () => {
            ...
            // ACTION: Create group
            const loadTestGroup = await apiDriver.createGroup(new CreateGroupRequestBuilder()
                .build(), user1.token);
        ...
        test('should handle multi-user notifications efficiently at scale', async () => {
            ...
            // ACTION: Create group with user1 as creator
            const scaleGroup = await apiDriver.createGroup(new CreateGroupRequestBuilder()
    ...
    describe('Comment Notifications', () => {
        ...
        test('creating an expense comment should trigger a notification for all group members', async () => {
            ...
            const expense = await apiDriver.createExpense(new CreateExpenseRequestBuilder()
                .withGroupId(group.id)
                .withAmount(100)
                .withPaidBy(user1.uid)
                .withParticipants([user1.uid, user2.uid])
