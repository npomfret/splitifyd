...
import type { ExpenseData, Group } from '../../../shared/shared-types';

export class PerformanceTestWorkers {
    constructor(private driver: ApiDriver) {}

    async createConcurrentExpenses(params: { users: User[]; group: Group; expensesPerUser: number; timeoutMs?: number }): Promise<{
        totalTime: number;
        succeeded: number;
        failed: number;
        ...
        const expenses = results.filter((r): r is PromiseFulfilledResult<ExpenseData> => r.status === 'fulfilled').map((r) => r.value);

        return { totalTime, succeeded, failed, expenses };
    }

    async createBalanceTestExpenses(params: { user1: User; user2: User; group: Group; expensesPerUserPair: number }): Promise<void> {
    ...
    async createLargeGroupExpenses(params: { group: Group; user1: User; user2: User; totalExpenses: number; batchSize: number }): Promise<{
        creationTime: number;
        ...
        await this.driver.pollGroupBalancesUntil(group.id, user1.token, (balance) => balance && Object.keys(balance.userBalances || {}).length > 0);
        const balanceTime = Date.now() - balanceStart;

        return { creationTime, retrievalTime, balanceTime };
    }

    async createComplexDebtGraph(params: { users: User[]; group: Group; expensesPerUser: number }): Promise<{
        ...
        const startTime = Date.now();
        await this.driver.pollGroupBalancesUntil(group.id, users[0].token, (balance) => balance && Object.keys(balance.userBalances || {}).length > 0);
        const balanceCalculationTime = Date.now() - startTime;

        return { totalExpenses, balanceCalculationTime };
    }

    async performRepeatedOperations(params: { group: Group; user: User; iterations: number }): Promise<void> {
        ...
        for (let i = 0; i < iterations; i++) {
            ...
            await this.driver.pollGroupBalancesUntil(group.id, user.token, (balance) => balance !== null && balance !== undefined);

            // Update expense
            await this.driver.updateExpense(
                expense.id,
                {
    ...
    async handleGroupMemberships(params: { busyUser: User; groupCount: number; expensesPerGroup: number; userSuffix: string }): Promise<{
        groupIds: string[];
        responseTime: number;
    }> {
        ...
        const balancePromises = groupIds.map((groupId) => this.driver.pollGroupBalancesUntil(groupId, busyUser.token, (balance) => balance !== null && balance !== undefined));
