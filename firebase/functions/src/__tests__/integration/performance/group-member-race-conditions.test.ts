...
describe('Group Member Race Conditions', () => {
    ...
    describe('Expenses persist when users join group', () => {
        test('should preserve expenses when single user joins group', async () => {
            ...
            const groupData = new GroupBuilder().withName(`Race Test Group ${uuidv4()}`).withDescription('Testing expense persistence').build();
        ...
        test('should preserve expenses when multiple users join simultaneously', async () => {
            ...
            const groupData = new GroupBuilder().withName(`Concurrent Join Test ${uuidv4()}`).build();
            const group = await driver.createGroup(groupData, users[0].token);
        ...
        test('should handle expense creation while user is joining', async () => {
            // Step 1: Create group
            const groupData = new GroupBuilder().withName(`Concurrent Operations Test ${uuidv4()}`).build();
            const group = await driver.createGroup(groupData, users[0].token);
        ...
        test('should preserve expenses during rapid successive joins', async () => {
            ...
            const groupData = new GroupBuilder().withName(`Rapid Join Test ${uuidv4()}`).build();
            const group = await driver.createGroup(groupData, users[0].token);
        ...
        test('should handle group updates while users are joining', async () => {
            // Step 1: Create group
            const groupData = new GroupBuilder().withName(`Update During Join Test ${uuidv4()}`).withDescription('Original description').build();
            const group = await driver.createGroup(groupData, users[0].token);
    ...
    describe('Data integrity during concurrent operations', () => {
        test('should maintain correct balances when users join group with expenses', async () => {
            // Step 1: Create group
            const group = await driver.createGroup(new GroupBuilder().withName(`Balance Test ${uuidv4()}`).build(), users[0].token);
