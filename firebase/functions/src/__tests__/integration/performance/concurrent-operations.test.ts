...
describe('Concurrent Operations and Transaction Integrity', () => {
    ...
    describe('Race Conditions', () => {
        test('should handle concurrent expense creation in same group', async () => {
            ...
            for (let i = 0; i < concurrentExpenses; i++) {
                ...
                expensePromises.push(
                    driver
                        .createExpense(expenseData, users[userIndex].token)
            ...
            const expenseIds = successfulExpenses.filter((e): e is { success: boolean; id: string; index: number } => 'id' in e).map((e) => e.id);
        ...
        test('should handle concurrent balance updates correctly', async () => {
            ...
            for (let i = 0; i < concurrentUpdates; i++) {
                ...
                updatePromises.push(driver.createExpense(expenseData, users[i % 3].token));
            }
            ...
            const concurrentExpenseDescriptions = finalExpenses.expenses.map((e: any) => e.description).filter((desc: string) => desc.startsWith('Concurrent Balance Update'));
        ...
        test('should handle concurrent group membership changes', async () => {
            ...
            const joinPromises = newUsers.map((user, index) =>
                driver
                    .joinGroupViaShareLink(shareResponse.linkId, user.token)
        ...
        test('should prevent duplicate concurrent joins by same user', async () => {
            ...
            const joinPromises = Array(joinAttempts)
                .fill(null)
                .map((_, index) =>
                    driver
                        .joinGroupViaShareLink(shareResponse.linkId, duplicateUser.token)
    ...
    describe('Transaction Integrity', () => {
        test('should validate split amounts match total in concurrent scenarios', async () => {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                invalidExpensePromises.push(
                    driver
                        .createExpense(expenseData, users[0].token)
        ...
        test('should handle partial failures in bulk operations gracefully', async () => {
            ...
            const mixedExpenses = [
                // Valid expense
                new ExpenseBuilder().withGroupId(testGroup.id).withDescription('Valid Expense 1').withAmount(50).withPaidBy(users[0].uid).withParticipants([users[0].uid, users[1].uid]).build(),
                ...
                new ExpenseBuilder()
                    ...
                    .build(),
                // Valid expense
                new ExpenseBuilder().withGroupId(testGroup.id).withDescription('Valid Expense 2').withAmount(75).withPaidBy(users[1].uid).withParticipants([users[0].uid, users[1].uid]).build(),
                // Invalid: empty participants
                new ExpenseBuilder().withGroupId(testGroup.id).withDescription('Invalid Expense - No Participants').withAmount(100).withPaidBy(users[0].uid).withParticipants([]).build(),
            ];
            ...
            const results = await Promise.all(
                mixedExpenses.map((expense, index) =>
                    driver
                        .createExpense(expense, users[0].token)
        ...
        test('should maintain balance consistency after failed operations', async () => {
            ...
            const baselineExpense = await driver.createExpense(
                new ExpenseBuilder().withGroupId(testGroup.id).withDescription('Baseline Expense').withAmount(100).withPaidBy(users[0].uid).withParticipants([users[0].uid, users[1].uid]).build(),
                users[0].token,
            );
        ...
        test('should handle concurrent deletes of same expense gracefully', async () => {
            ...
            const deletePromises = users.slice(0, 3).map((user) =>
                driver
                    .deleteExpense(expenseToDelete.id, user.token)
            ...
            await expect(driver.getExpense(expenseToDelete.id, users[0].token)).rejects.toThrow(/not found|deleted|404/i);
