...
describe('Balance Calculation Integration Test', () => {
    ...
    it('should correctly calculate balances for multi-user expense sharing', async () => {
        ...
        const unexpectedDebts = debts.filter((debt) => !(debt.from.userId === user1.uid && debt.to.userId === user3.uid) && !(debt.from.userId === user2.uid && debt.to.userId === user3.uid));
    ...
    it('should correctly handle complex debt simplification with circular debts', async () => {
        ...
        const groupData = new CreateGroupRequestBuilder().withName('Complex Debt Test Group').build();
        const createGroupResponse = await apiDriver.createGroup(groupData, user1.token);
        groupId = createGroupResponse.id;
        ...
        const complexBalances = await apiDriver.pollGroupBalancesUntil(
            ...
            (balances) => {
                ...
                return balances.userBalances[user1.uid] && balances.userBalances[user2.uid] && balances.userBalances[user3.uid] && balances.simplifiedDebts && balances.simplifiedDebts.length > 0;
            },
    ...
    it('should handle zero-sum scenario with no debts', async () => {
        ...
        // Create group
        const groupData = new CreateGroupRequestBuilder().withName('Zero Sum Test Group').build();
        const createGroupResponse = await apiDriver.createGroup(groupData, user1.token);
        groupId = createGroupResponse.id;
    ...
    it('should correctly handle multi-currency debt simplification', async () => {
        ...
        // Create group
        const groupData = new CreateGroupRequestBuilder().withName('International Travel Group').build();
        const createGroupResponse = await apiDriver.createGroup(groupData, user1.token);
        groupId = createGroupResponse.id;
        ...
        const usdDebtFromUser2 = debtsByCurrency.USD.find((d) => d.from.userId === user2.uid && d.to.userId === user1.uid);
        expect(usdDebtFromUser2).toBeDefined();
        expect(usdDebtFromUser2!.amount).toBe(100);

        const usdDebtFromUser3 = debtsByCurrency.USD.find((d) => d.from.userId === user3.uid && d.to.userId === user1.uid);
        ...
        const eurDebtFromUser1 = eurDebts.find((d) => d.from.userId === user1.uid && d.to.userId === user2.uid);
        expect(eurDebtFromUser1).toBeDefined();
        expect(eurDebtFromUser1!.amount).toBe(70);

        const eurDebtFromUser3 = eurDebts.find((d) => d.from.userId === user3.uid && d.to.userId === user2.uid);
        ...
        Object.entries(multiCurrencyBalances.balancesByCurrency).forEach(([, balances]) => {
            const total = Object.values(balances).reduce((sum, userBalance) => sum + userBalance.netBalance, 0);
    ...
    it('should handle edge case with single currency but multiple exchange rates over time', async () => {
        ...
        const groupData = new CreateGroupRequestBuilder().withName('Exchange Rate Test Group').build();
        const createGroupResponse = await apiDriver.createGroup(groupData, user1.token);
        groupId = createGroupResponse.id;
