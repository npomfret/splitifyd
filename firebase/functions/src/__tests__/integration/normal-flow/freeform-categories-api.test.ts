...
describe('Freeform Categories API Integration', () => {
  ...
  beforeEach(async () => {
    const groupData = new GroupBuilder()
      .withName(`Freeform Categories Test Group ${uuidv4()}`)
      .withMembers(users)
      .build();
  ...
  describe('Expense Creation with Custom Categories', () => {
    ...
    test('should create expense with category containing numbers and safe unicode characters', async () => {
      ...
      expect(response.amount).toBe(999.00);
    });
  ...
  describe('Category Validation Edge Cases', () => {
    test('should reject expense with category exceeding 50 characters', async () => {
      ...
      ).rejects.toThrow(/between 1 and 50 characters|INVALID_CATEGORY/i);
    });
  ...
  describe('Backward Compatibility', () => {
    ...
    test('should accept previously "invalid" categories that are now valid', async () => {
      const previouslyInvalidCategories = [
        'FOOD', 'Food & Dining', 'travel-expenses', 'auto_maintenance',
        'Coffee Shop', 'gas-station', 'medical-appointment'
      ];
      ...
      for (const category of previouslyInvalidCategories) {
        ...
        expect(response.category).toBe(category);
        expect(response.amount).toBe(15.00);
      }
    });
  });
