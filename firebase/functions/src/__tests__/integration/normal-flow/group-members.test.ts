...
describe('Group Members Integration Tests', () => {
  ...
  const createGroupWithMembers = async (driver: ApiDriver, users: User[]): Promise<string> => {
    const groupData = new GroupBuilder()
      ...
      .withDescription('Test group for member operations')
      .build();
  ...
  describe('getGroupMembers', () => {
    it('should return all group members', async () => {
      ...
      expect(response).toMatchObject({
        members: expect.arrayContaining([
          expect.objectContaining({ uid: users[0].uid }),
          expect.objectContaining({ uid: users[1].uid }),
          expect.objectContaining({ uid: users[2].uid }),
        ]),
        totalCount: 3,
        hasMore: false,
      });
    ...
    it('should throw error if user is not authenticated', async () => {
      ...
      await expect(driver.getGroupMembers(groupId, 'invalid-token'))
        .rejects.toThrow();
    });
    ...
    it('should throw FORBIDDEN if user is not a member', async () => {
      ...
      await expect(driver.getGroupMembers(groupId, nonMember.token))
        .rejects.toThrow();
    });
  });
  ...
  describe('leaveGroup', () => {
    ...
    it('should prevent the creator from leaving', async () => {
      ...
      await expect(driver.leaveGroup(groupId, users[0].token))
        .rejects.toThrow(/Group creator cannot leave/);
    });
    ...
    it('should prevent leaving with outstanding balance', async () => {
      ...
      await expect(driver.leaveGroup(groupId, memberWithDebt.token))
        .rejects.toThrow(/Cannot leave group with outstanding balance/);
    });
  ...
  describe('removeGroupMember', () => {
    ...
    it('should prevent non-creator from removing members', async () => {
      ...
      await expect(driver.removeGroupMember(groupId, memberToRemove.uid, nonCreator.token))
    ...
    it('should prevent removing member with outstanding balance', async () => {
      ...
      await expect(driver.removeGroupMember(groupId, memberWithDebt.uid, creator.token))
        .rejects.toThrow(/Cannot remove member with outstanding balance/);
    });
  ...
  describe('Complex scenarios', () => {
    ...
    it('should prevent access after leaving group', async () => {
      ...
      await expect(driver.getGroupMembers(groupId, memberToLeave.token))
        .rejects.toThrow();
    });
