 ...
 */
export const checkTimestampConflict = async (transaction: admin.firestore.Transaction, docRef: admin.firestore.DocumentReference, originalTimestamp: Timestamp): Promise<void> => {
 ...
 */
export const withOptimisticLocking = async <T>(operation: (transaction: admin.firestore.Transaction) => Promise<T>): Promise<T> => {
 ...
 */
export const withOptimisticLockingRetry = async <T>(operation: (transaction: admin.firestore.Transaction) => Promise<T>, maxRetries: number = 3, operationName?: string): Promise<T> => {
    let attempts = 0;
    ...
    while (attempts < maxRetries) {
        ...
        try {
            ...
            if (error instanceof Error && (error.message.includes('CONCURRENT_UPDATE') || error.message.includes('Transaction failed'))) {
                if (isLastAttempt) {
 ...
 */
export const updateWithServerTimestamp = (docRef: admin.firestore.DocumentReference, updates: any): Promise<admin.firestore.WriteResult> => {
 ...
 */
export const updateWithServerTimestampTransaction = (transaction: admin.firestore.Transaction, docRef: admin.firestore.DocumentReference, updates: any): void => {
    transaction.update(docRef, {
        ...updates,
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
    });
