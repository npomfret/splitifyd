...
export const acceptPolicy = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!policyId || !versionHash) {
      throw new ApiError(HTTP_STATUS.BAD_REQUEST, 'INVALID_REQUEST', 
        'policyId and versionHash are required');
    }
    ...
    if (!policyDoc.exists) {
      throw new ApiError(HTTP_STATUS.NOT_FOUND, 'POLICY_NOT_FOUND', 
    ...
    if (!policyData.versions[versionHash]) {
      throw new ApiError(HTTP_STATUS.BAD_REQUEST, 'INVALID_VERSION_HASH', 
...
export const acceptMultiplePolicies = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!acceptances || !Array.isArray(acceptances) || acceptances.length === 0) {
      throw new ApiError(HTTP_STATUS.BAD_REQUEST, 'INVALID_REQUEST', 
        'acceptances array is required and must not be empty');
    }
    ...
    for (const acceptance of acceptances) {
      ...
      if (!policyId || !versionHash) {
        throw new ApiError(HTTP_STATUS.BAD_REQUEST, 'INVALID_REQUEST', 
          'Each acceptance must have policyId and versionHash');
      }
