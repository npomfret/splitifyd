...
describe('useExpenseForm', () => {
  ...
  it('provides access to group and members data', () => {
    ...
    expect(result.current.members).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ uid: 'user1', displayName: 'Alice Johnson' }),
        expect.objectContaining({ uid: 'user2', displayName: 'Bob Smith' })
      ])
    );
  });
  ...
  describe('Form Interactions', () => {
    ...
    it('calls split amount update method', () => {
      ...
      act(() => {
        result.current.updateSplitAmount('user1', 15.00);
      });

      expect(expenseFormStore.updateSplitAmount).toHaveBeenCalledWith('user1', 15.00);
    });
  ...
  describe('Recent Amounts', () => {
    it('provides getRecentAmounts function', () => {
      ...
      expect(recentAmounts).toEqual([25.50, 45.00, 12.75]);
    });
  });
