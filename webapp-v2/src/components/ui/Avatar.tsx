...
export function Avatar({ displayName, userId, size = 'md', themeColor, photoURL }: EnhancedAvatarProps) {
  ...
  const patternStyle = userTheme.pattern !== 'solid' 
    ? getPatternStyle(userTheme.pattern, backgroundColor) 
    : {};
  ...
  return (
    ...
    >
      <span 
        className={`${sizeClasses.text} font-medium relative z-10`}
      ...
      {userTheme.pattern !== 'solid' && (
        <div 
          className="absolute inset-0 pointer-events-none opacity-20"
          style={getPatternOverlay(userTheme.pattern)}
        />
      )}
    </div>
  );
