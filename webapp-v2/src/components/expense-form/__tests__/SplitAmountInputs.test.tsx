...
describe('SplitAmountInputs', () => {
  ...
  const mockSplits = [
    { userId: 'user1', amount: 20.00, percentage: 50 },
    { userId: 'user2', amount: 20.00, percentage: 50 }
  ];
  ...
  it('returns null when amount is zero or negative', () => {
    const { container } = render(
      <SplitAmountInputs {...defaultProps} amount="0" />
    );
    expect(container.firstChild).toBeNull();

    const { container: container2 } = render(
      <SplitAmountInputs {...defaultProps} amount="-10" />
    );
  ...
  describe('Equal Split Mode', () => {
    ...
    it('handles missing member data gracefully', () => {
      const splitsWithUnknownUser = [
        { userId: 'user1', amount: 20.00 },
        { userId: 'unknown-user', amount: 20.00 }
      ];
      ...
      render(
        <SplitAmountInputs 
          {...defaultProps} 
          splitType="equal"
          participants={['user1', 'unknown-user']}
          splits={splitsWithUnknownUser}
        />
      );
  ...
  describe('Exact Split Mode', () => {
    ...
    it('calls updateSplitAmount when input values change', () => {
      ...
      expect(mockUpdateSplitAmount).toHaveBeenCalledWith('user1', 25.50);
    });
  ...
  describe('Percentage Split Mode', () => {
    ...
    it('displays percentage total in red when percentages do not sum to 100', () => {
      const mismatchedSplits = [
        { userId: 'user1', amount: 16.00, percentage: 40 },
        { userId: 'user2', amount: 24.00, percentage: 60 }
      ];
  ...
  describe('Amount Conversion', () => {
    ...
    it('handles invalid string amount', () => {
      const { container } = render(
        <SplitAmountInputs {...defaultProps} amount="invalid" />
      );
      expect(container.firstChild).toBeNull();
    });
  });
