...
function parseMarkdown(markdown: string): string {
    ...
    return (
        safeInput
            // Headers
            .replace(/^### (.*$)/gim, '<h3 class="text-lg font-semibold text-text-primary mb-2">$1</h3>')
            .replace(/^## (.*$)/gim, '<h2 class="text-xl font-bold text-text-primary mb-4">$1</h2>')
            .replace(/^# (.*$)/gim, '<h1 class="text-2xl font-bold text-text-primary mb-6">$1</h1>')
            // Bold and italic
            ...
            .replace(/^(.*)$/gim, '<p class="text-text-primary mb-3">$1</p>')
            // Clean up empty paragraphs and fix nesting
            .replace(/<p class="text-text-primary mb-3"><\/p>/g, '')
            .replace(/<p class="text-text-primary mb-3">(<h[1-6])/g, '$1')
            .replace(/(<\/h[1-6]>)<\/p>/g, '$1')
            .replace(/<p class="text-text-primary mb-3">(<ul)/g, '$1')
            .replace(/(<\/ul>)<\/p>/g, '$1')
    );
}
