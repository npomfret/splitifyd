...
describe('getGroupDisplayName', () => {
    it('returns group display name when available', () => {
        const result = getGroupDisplayName({ groupDisplayName: 'Scout Leader', displayName: 'Alex M.' });
        expect(result).toBe('Scout Leader');
    });

    it('trims whitespace from group display name', () => {
        const result = getGroupDisplayName({ groupDisplayName: '  Fire Team  ', displayName: 'Taylor' });
