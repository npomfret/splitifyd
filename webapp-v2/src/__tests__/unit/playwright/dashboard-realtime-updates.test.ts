...
import { createMockFirebase, mockFullyAcceptedPoliciesApi, mockGroupsApi } from '../../utils/mock-firebase-service';
...
test.describe('Dashboard Real-time Notifications', () => {
    ...
    test('should refresh dashboard when notification received (covers balance/transaction/group changes)', async ({ pageWithLogging: page }) => {
        const dashboardPage = new DashboardPage(page);

        const initialGroup = GroupDTOBuilder.groupForUser(testUser.uid).withId('group-test').withName('Test Group').build();
        const updatedGroup = GroupDTOBuilder.groupForUser(testUser.uid).withId('group-test').withName('Updated Group').build();
        ...
        await mockFirebase.triggerNotificationUpdate(testUser.uid, UserNotificationDocumentBuilder.withBaseline('group-test', 1).withLastModified(new Date()).build());
        ...
        await mockFirebase.triggerNotificationUpdate(testUser.uid, UserNotificationDocumentBuilder.withGroupDetailsChange('group-test', 2).withLastModified(new Date()).build());

        // Verify dashboard refreshed
        await dashboardPage.waitForGroupToAppear('Updated Group');
    });

    test('should handle rapid concurrent notifications without race conditions', async ({ pageWithLogging: page }) => {
        const dashboardPage = new DashboardPage(page);
    ...
    test('should handle invalid notifications gracefully', async ({ pageWithLogging: page }) => {
        const dashboardPage = new DashboardPage(page);
        ...
        await mockFirebase.triggerNotificationUpdate(testUser.uid, { changeVersion: 2, groups: { 'valid-group': { lastGroupDetailsChange: new Date() } } });

        // Send notification with invalid group ID
        await mockFirebase.triggerNotificationUpdate(testUser.uid, new UserNotificationDocumentBuilder().withChangeVersion(3).withGroupDetails('non-existent-id', 1).withGroupDetails('valid-group', 1).build());
