...
test.describe('EnhancedGroupsStore - Core Functionality (Converted)', () => {
    ...
    test('should fetch and store groups - verified via dashboard display', async ({ page }) => {
        ...
        const criticalErrors = jsErrors.filter((error) => error.includes('groups') && (error.includes('store') || error.includes('factory')));
    ...
    test('should create new group and add to list - verified via UI creation flow', async ({ page }) => {
        ...
        const storeErrors = jsErrors.filter((error) => error.includes('createGroup') || (error.includes('groups') && error.includes('method')));
    ...
    test('should handle API errors gracefully - verified via error UI display', async ({ page }) => {
        ...
        const criticalErrors = jsErrors.filter((error) => error.includes('store') && error.includes('groups') && error.includes('error'));
        expect(criticalErrors).toHaveLength(0);
    });
});
