...
async function loadCurrencies(): Promise<Currency[]> {
  ...
  loadingPromise = import('../../../../firebase/functions/src/static-data/currencies.json')
    .then((module) => {
      currenciesCache = module.default as Currency[];
      currencyMapCache = new Map<string, Currency>(
        currenciesCache.map(currency => [currency.acronym, currency])
      );
      return currenciesCache;
    });
...
const essentialCurrencyMap = new Map<string, Currency>(
  ESSENTIAL_CURRENCIES_DATA.map(currency => [currency.acronym, currency])
);
