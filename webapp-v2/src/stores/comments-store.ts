    ...
    async #fetchCommentsViaApi(target: CommentsStoreTarget, isRefresh: boolean = false) {
        ...
        try {
            ...
            if (target.type === 'group') {
                response = await apiClient.getGroupComments(target.groupId, this.#apiCursor ? { cursor: this.#apiCursor } : undefined);
            } else {
                response = await apiClient.getExpenseComments(target.expenseId, this.#apiCursor ? { cursor: this.#apiCursor } : undefined);
            }
