...
- Hardened the Playwright page objects: replaced CSS-class locators (e.g., `.animate-spin`, `.space-y-0.5`, `.fixed.bottom-4.right-4`) with accessible anchors and new `data-testid` hooks (`loading-spinner`, `share-link-toast`, `members-scroll-container`). Updated the corresponding UI components and unit/e2e tests so selectors now target visible headings, roles, or purposeful test IDs rather than Tailwind classes.
